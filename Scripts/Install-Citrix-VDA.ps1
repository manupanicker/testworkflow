...
param(
    ...
    [string]$InstallerName = 'VDAWorkstationSetup_x64.exe'
)
...
Write-Host "VDA installer: $($i.FullName)"
$p=Start-Process $i.FullName -ArgumentList @('/quiet','/noreboot') -Wait -PassThru -NoNewWindow
Write-Host "Installer exit code: $($p.ExitCode)"
if($p.ExitCode -eq 0){Write-Host 'INSTALL_COMPLETE'} elseif($p.ExitCode -in @(3,14,3010,1641)){Write-Host "REBOOT_REQUIRED: exit code $($p.ExitCode)"} else {throw "FATAL: VDA installer failed with exit code $($p.ExitCode)"}
